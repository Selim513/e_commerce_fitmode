...
//------------Add product-----------------
  ...
  try {
    ...
    emit(CartSuccessState(succMessage: 'The product has been successfully added to the cart'));
  } catch (e) {
    emit(CartFailureState(errMessage: e.toString()));
  }
}
...
//---------get product--------------
  ...
  Future<void> deleteCartItem(int productId) async {
    try {
      ...
      await fireStore.collection('cart').doc(productId.toString()).delete();
  ...
  try {
    await fireStore.collection('cart').doc(productId.toString()).update({
